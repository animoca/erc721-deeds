...
describe('ERC721 Trusted Agent', function () {
  ...
  describe('wrapWithSafeTransfer(uint256)', function () {
    beforeEach(async function () {
      await this.underlyingToken.approve(this.trustedAgent.address, tokenId, {from: owner1});
      this.receipt = await this.trustedAgent.wrapWithSafeTransfer(tokenId, {from: owner1});
    });
  ...
  describe('wrapWithFunctionCall(uint256)', function () {
    beforeEach(async function () {
      await this.underlyingToken.approve(this.trustedAgent.address, tokenId, {from: owner1});
      this.receipt = await this.trustedAgent.wrapWithFunctionCall(tokenId, {from: owner1});
    });

    it('transfers the token to the Trusted Agent', async function () {
      await expectEvent.inTransaction(this.receipt.tx, this.underlyingToken, 'Transfer', {
        _from: owner1,
        _to: this.trustedAgent.address,
        _tokenId: tokenId,
      });
    });
  ...
  describe('unwrap(uint256)', function () {
    function behaveLikeDeedDestruction(deedOwner, otherAccount) {
      it('reverts if the sender is not the deed owner', async function () {
        await expectRevert(this.trustedAgent.unwrap(tokenId, {from: otherAccount}), 'TrustedAgent: not deed owner');
      });

      context('when successful', function () {
        beforeEach(async function () {
          this.receipt = await this.trustedAgent.unwrap(tokenId, {from: deedOwner});
        });
    ...
    context('from another owner after deed transfer', function () {
      beforeEach(async function () {
